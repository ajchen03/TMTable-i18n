...
// data = a function returning the content (actually HTML)
	...
	Vue.component('challenge', {
		...
		<div v-if="tmp[layer].challenges && tmp[layer].challenges[data]!== undefined && tmp[layer].challenges[data].unlocked && !(options.hideChallenges && maxedChallenge(layer, [data]) && !inChallenge(layer, [data]))"
			...
			<span v-else>
				<span v-html="tmp[layer].challenges[data].challengeDescriptionEN"></span>
				{{text('systemComponent.challenge.goal')}}:  <span v-if="tmp[layer].challenges[data].goalDescription" v-html="tmp[layer].challenges[data].goalDescription"></span><span v-else>{{format(tmp[layer].challenges[data].goal)}} {{tmp[layer].challenges[data].currencyDisplayName ? tmp[layer].challenges[data].currencyDisplayName : modInfo.pointsName}}</span><br>
				{{text('systemComponent.challenge.reward')}}: <span v-html="tmp[layer].challenges[data].rewardDescription></span><br>
				<span v-if="layers[layer].challenges[data].rewardDisplay!==undefined">{{text('systemComponent.challenge.reward')}}: <span v-html="(tmp[layer].challenges[data].rewardDisplay) ? (run(layers[layer].challenges[data].rewardDisplay, layers[layer].challenges[data])) : format(tmp[layer].challenges[data].rewardEffect)"></span></span>
			</span>
	...
	Vue.component('upgrade', {
		...
		<button v-if="tmp[layer].upgrades && tmp[layer].upgrades[data]!== undefined && tmp[layer].upgrades[data].unlocked" :id='"upgrade-" + layer + "-" + data' v-on:click="buyUpg(layer, data)" v-bind:class="{ [layer]: true, upg: true, bought: hasUpgrade(layer, data), locked: (!(canAffordUpgrade(layer, data))&&!hasUpgrade(layer, data)), can: (canAffordUpgrade(layer, data)&&!hasUpgrade(layer, data))}"
			...
			<span v-else>
				...
				<span v-if="layers[layer].upgrades[data].effectDisplay"><br>{{text('systemComponent.upgrade.currently')}}: <span v-html="run((layers[layer].upgrades[data].effectDisplay, layers[layer].upgrades[data])"></span></span>
				<br><br>{{text('systemComponent.upgrade.cost')}}: {{ formatWhole(tmp[layer].upgrades[data].cost) }} {{(tmp[layer].upgrades[data].currencyDisplayName ? tmp[layer].upgrades[data].currencyDisplayName : (tmp[layer].resource))}}
			</span>	
			</button>
		`
	})
	...
	Vue.component('milestone', {
		...
		<td v-if="tmp[layer].milestones && tmp[layer].milestones[data]!== undefined && milestoneShown(layer, data) && tmp[layer].milestones[data].unlocked" v-bind:style="[tmp[layer].milestones[data].style]" v-bind:class="{milestone: !hasMilestone(layer, data), tooltipBox: true, milestoneDone: hasMilestone(layer, data)}">
			<br><h3 v-if="tmp[layer].milestones[data].requirementDescription" v-html="tmp[layer].milestones[data].requirementDescription"></h3><br>
	...
	Vue.component('toggle', {
		props: ['layer', 'data'],
		template: `
		<button class="smallUpg can" v-bind:style="{'background-color': tmp[data[0]].color}" v-on:click="toggleAuto(data)">{{player[data[0]][data[1]]?(text('systemComponent.toggle.on')):(text('systemComponent.toggle.off')}}</button>
		`
	})
	...
	Vue.component('main-display', {
		props: ['layer', 'data'],
		template: `
		<div><span v-if="player[layer].points.lt('1e1000')">{{text('systemComponent.main-display.youHave')}} </span><h2 v-bind:style="{'color': tmp[layer].color, 'text-shadow': '0px 0px 10px ' + tmp[layer].color}">{{data ? format(player[layer].points, data) : formatWhole(player[layer].points)}}</h2> {{tmp[layer].resource}}<span v-if="layers[layer].effectDescription">, <span v-html="run(?layers[layer].effectDescription, layers[layer])"></span></span><br><br></div>
		`
	})
	...
	Vue.component('resource-display', {
		...
		<div style="margin-top: -13px">
			<span v-if="tmp[layer].baseAmount"><br>{{text('systemComponent.resource-display.baseAmount')}} {{formatWhole(tmp[layer].baseAmount)}} {{tmp[layer].baseResource}}</span>
			<span v-if="tmp[layer].passiveGeneration"><br>{{text('systemComponent.resource-display.passiveGeneration')}} {{format(tmp[layer].resetGain.times(tmp[layer].passiveGeneration))}} {{tmp[layer].resource}}{{text('systemComponent.resource-display.afterPassiveGeneration')}}</span>
			<br><br>
			<span v-if="tmp[layer].showBest">{{text('systemComponent.resource-display.showBest')}} {{formatWhole(player[layer].best)}} {{tmp[layer].resource}}<br></span>
			<span v-if="tmp[layer].showTotal">{{(text('systemComponent.resource-display.showTotal')}} {{formatWhole(player[layer].total)}} {{tmp[layer].resource}}<br></span>
		</div>
		`
	})
	...
	Vue.component('achievement', {
		...
		<div v-if="tmp[layer].achievements && tmp[layer].achievements[data]!== undefined && tmp[layer].achievements[data].unlocked" v-bind:class="{ [layer]: true, achievement: true, tooltipBox:true, locked: !hasAchievement(layer, data), bought: hasAchievement(layer, data)}"
			...
			<tooltip :text="
			(tmp[layer].achievements[data].tooltip == '') ? false : hasAchievement(layer, data) ? (tmp[layer].achievements[data].doneTooltip ? tmp[layer].achievements[data].doneTooltip : (tmp[layer].achievements[data].tooltip ? (tmp[layer].achievements[data].tooltip) : (text('systemComponent.achievement.doneTooltip'))))
			: (tmp[layer].achievements[data].goalTooltip ? tmp[layer].achievements[data].goalTooltip : (tmp[layer].achievements[data].tooltip ? (tmp[layer].achievements[data].tooltip) : (text('systemComponent.achievement.locked')))
		"></tooltip>
